...
eve(alarm(var_L,var_R)):-assertz(pending_alarm(var_L,var_R)).

evi(pending_alarm(var__L,var__R)):-busy,write('SAFETY: busy, alarm queued'),nl.

evi(pending_alarm(var_L,var_R)):- \+busy,assertz(busy),retract(pending_alarm(var_L,var_R)),write('SAFETY: handling alarm for '),write(var_R),nl,a(message(guardTeam,send_message(dispatch_guard(var_R,var_L),var_Me))),a(message(evacuator,send_message(start_evacuation(var_R,var_L),var_Me))).

eve(guard_confirm(var_R)):-write('SAFETY: guard confirmed for '),write(var_R),nl.

eve(firefighters_called(var_R)):-write('SAFETY: firefighters called for room '),write(var_R),nl.

eve(room_cleared(var_R)):-write('SAFETY: room cleared for '),write(var_R),nl.

eve(room_safe(var_R)):-write('SAFETY: evacuation completed in '),write(var_R),nl,retractall(busy),trigger_next_dispatch.

trigger_next_dispatch:-pending_alarm(var_L2,var_R2),retract(pending_alarm(var_L2,var_R2)),assertz(pending_alarm(var_L2,var_R2)).

trigger_next_dispatch:- \+pending_alarm(var__,var__),write('SAFETY: no more alarms'),nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_334875,var_Ontology,_334879),_334869),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_334913)),a(message(var_Ag,propose(var_A,[_334913],var_AgI))),retractall(ext_agent(var_Ag,_334951,var_Ontology,_334955)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_334749,var_Ontology,_334753),_334743),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_334819,var_Ontology,_334823)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_334637,var_Ontology,_334641),_334631),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_334693,var_Ontology,_334697)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_334201),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_334235),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_334017).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_333865),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_333813),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_333689,_333691)),clause(agente(_333711,_333713,_333715,var_S),_333707),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_333483,_333485,_333487,var_S),_333479),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_333583,[]),repeat,read(_333583,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_333583).
